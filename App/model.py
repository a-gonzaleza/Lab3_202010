...
def newCatalog():
    ...
    Inicializa el catálogo de peliculas. Retorna el catalogo inicializado.
    """
    catalog = {'moviesList':None, 'authors':None, 'booksMap': None}
    catalog['moviesList'] = lt.newList("ARRAY_LIST")
    catalog['moviesMap'] = map.newMap (1001, maptype='CHAINING')#2000 movies
    catalog['authors'] = map.newMap (12007, maptype='PROBING') #5841 authors
    return catalog
...
def newMovie (row):
    ...
    """
    movie = {"movie_id": row['id'], "title":row['original_title'], "vote_average":row['vote_average'], "vote_count":row['vote_count']}
    return movie
